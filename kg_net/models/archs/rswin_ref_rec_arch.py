import kg_net.models.archs.arch_util as arch_util
import torch
import torch.nn as nn
import torch.nn.functional as F
# from kg_net.models.archs.DCNv2.dcn_v2 import DCN_sep_pre_multi_offset_flow_similarity as DynAgg
from kg_net.models.archs.dcn_v2 import DCN_sep_pre_multi_offset_flow_similarity as DynAgg
import torch.utils.checkpoint as checkpoint
from timm.models.layers import DropPath, to_2tuple, trunc_normal_
import math
from kg_net.models.archs.RW.LRMSA import MixedAttention
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        shortcut = x
        x = self.norm1(x)
        attn_lr = self.mixed_attn(x, x_size)
        x = x.view(B, H, W, C)
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        if hasattr(self, 'patch_embed'):   # hasattr():
...
class ContentExtractor(nn.Module):
    ...
    def __init__(self, in_nc=2, out_nc=2, nf=64, n_blocks=16):  # gmv3
...
class DataConsistencyInKspace_I(nn.Module):
    ...
    def perform(self, x, k0, mask):
        ...
        if x.dim() == 4:  # input is 2D
            x = x.permute(0, 2, 3, 1).contiguous()   # [n,w,h,2]
            k0 = k0.contiguous()
            mask = mask.contiguous()
        else:
            raise ValueError("error in data consistency layer!")
        x = torch.view_as_complex(x)   # [w,h]
        k0 = torch.view_as_complex(k0)
        # import pdb;pdb.set_trace()
        pred_k = FFT2D(x)
        pred_k0 = FFT2D(k0)
        k = torch.view_as_real(pred_k)  # [n,w,h,2]
        k_full = torch.view_as_real(pred_k0)

        out = data_consistency(k, k_full, mask, self.noise_lvl)
        out =  torch.view_as_complex(out)
        out_dc = IFFT2D(out)  # compelx
...
class SwinUnetv3RestorationNet(nn.Module):
    def __init__(self, ngf=64, n_blocks=16, groups=8, embed_dim=64, depths=(8,8), num_heads=(8,8), window_size=8, use_checkpoint=False):
        ...
        self.dc_image = DataConsistencyInKspace_I(noise_lvl=None)  # gmv2
        self.dw = cnn_denoiser(n_layers=5)
        self.k_iters = 5
    ...
    def forward(self, x_full, mask256, x, kd, pre_offset_flow_sim, img_ref_feat):

        base = x.contiguous()
        k0 = x_full.permute(0, 2, 3, 1)
        # content_feat = self.content_extractor(base)
        upscale_restore1, kd_new = self.dyn_agg_restore(base, kd, pre_offset_flow_sim, img_ref_feat)
        pre_out = upscale_restore1 + base

        for k in range(self.k_iters):
            z_k = self.dw(pre_out)
            pre_out = z_k
            # pre_out = self.dc_image(z_k, k0, mask256)
        return pre_out, kd_new
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def __init__(self,
        ...
        self.unet_head = nn.Conv2d(2, ngf, kernel_size=3, stride=1, padding=1) # gmv3
        ...
        self.down_body_large = SwinBlock(img_size=160, embed_dim=ngf,  # gmv3?
        ...
        self.up_tail_large = nn.Sequential(
            ...
            nn.LeakyReLU(0.1, True),
            nn.Conv2d(ngf // 2, 2, kernel_size=3, stride=1, padding=1))  # gmv2

        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

        self.x_agg_1 = nn.Sequential(
            nn.Conv2d(ngf + 64, ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
        self.x_agg_2 = nn.Sequential(
            nn.Conv2d(ngf + 128, ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
        self.x_agg_3 = nn.Sequential(
            nn.Conv2d(ngf + 256, ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
        self.x_agg_4 = nn.Sequential(
            nn.Conv2d(ngf + 128, ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
        self.x_agg_5 = nn.Sequential(
            nn.Conv2d(ngf + 64, ngf, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
    ...
    def forward(self, base, kd, pre_offset_flow_sim, img_ref_feat):
        ...
        if len(kd) == 5:
            x1_agg = torch.cat([x0, kd['down_1']], 1)
            x1_agg = self.x_agg_1(x1_agg)
        else:
            x1_agg = torch.cat([x0, kd['relu1_1']], 1)
            x1_agg = self.x_agg_1(x1_agg)

        h = torch.cat([x1_agg, down_relu1_swapped_feat], 1)
        ...
        if len(kd) == 5:
            x2_agg = torch.cat([x1, kd['down_2']], 1)
            x2_agg = self.x_agg_2(x2_agg)
        else:
            x2_agg = torch.cat([x1, kd['relu2_1']], 1)
            x2_agg = self.x_agg_2(x2_agg)

        h = torch.cat([x2_agg, down_relu2_swapped_feat], 1)
        ...
        if len(kd) == 5:
            x3_agg = torch.cat([x2, kd['swapped_3']], 1)
            x3_agg = self.x_agg_3(x3_agg)
        else:
            x3_agg = torch.cat([x2, kd['relu3_1']], 1)
            x3_agg = self.x_agg_3(x3_agg)

        # small scale
        h = torch.cat([x3_agg, relu3_swapped_feat], 1)
        ...
        if len(kd) == 5:
            x4_agg = torch.cat([x, kd['swapped_2']], 1)
            x4_agg = self.x_agg_4(x4_agg)
        else:
            x4_agg = torch.cat([x, kd['relu2_1']], 1)
            x4_agg = self.x_agg_4(x4_agg)
        # medium scale
        h = torch.cat([x4_agg+x1, relu2_swapped_feat], 1)
        ...
        if len(kd) == 5:
            x5_agg = torch.cat([x, kd['swapped_1']], 1)
            x5_agg = self.x_agg_5(x5_agg)
        else:
            x5_agg = torch.cat([x, kd['relu1_1']], 1)
            x5_agg = self.x_agg_5(x5_agg)

        # large scale
        h = torch.cat([x5_agg+x0, relu1_swapped_feat], 1)
        h = self.up_head_large(h)
        h = self.up_body_large(h) + x
        x = self.up_tail_large(h)
        new_kd = {}
        new_kd['down_1'] = down_relu1_swapped_feat
        new_kd['down_2'] = down_relu2_swapped_feat
        new_kd['swapped_3'] = relu3_swapped_feat
        new_kd['swapped_2'] = relu2_swapped_feat
        new_kd['swapped_1'] = relu1_swapped_feat
